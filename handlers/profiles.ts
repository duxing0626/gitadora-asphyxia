...
import { findInheritablePlayerInfo, getDefaultPlayerInfo, PlayerInfo } from "../models/playerinfo";
import { sheetToCardEntry } from "../models/playerboard";
...
export const getPlayer: EPR = async (info, data, send) => {
  ...
  // The background ("sheet") has no field of its own in the wire protocol — it's just another
  // sticker entry, placed first so it renders behind whatever's in the numbered slots. See
  // models/playerboard.ts:sheetToCardEntry.
  const boardCard = [
    ...(name.sheet != null ? [sheetToCardEntry(name.sheet)] : []),
    ...(_.isArray(name.card) ? name.card : []),
  ];
  const sticker: PlayerStickerResponse[] = getPlayerStickerResponse(boardCard);
  const playinfo: PlayerPlayInfoResponse = getPlayerPlayInfoResponse(profile);

  const playerData: any = {
    playerboard: {
      index: K.ITEM('s32', 1),
      is_active: K.ITEM('bool', boardCard.length > 0 ? 1 : 0),
      sticker,
    },
    player_info: {
...
async function registerUser(refid: string, version: string, id = _.random(0, 99999999)) {
  ...
  const inherited = await findInheritablePlayerInfo(refid);
  if (inherited) {
    defaultInfo.name = inherited.name;
    defaultInfo.title = inherited.title;
  }
